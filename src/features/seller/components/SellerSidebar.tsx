...
function NavLink({ item, active }: { item: SellerNavItem; active: boolean }) {
...
/** Dark, grouped seller navigation — structural twin of AdminSidebar, scoped to seller-relevant sections only. */
export function SellerSidebar({ user, pathname, onNavigate, onClose }: SellerSidebarProps) {
  ...
  return (
    <div className="flex h-full flex-col select-none border-r border-sidebar-border bg-sidebar-background">
      <div className="flex h-16 flex-shrink-0 items-center justify-between border-b border-sidebar-border px-5">
        <Link href={ROUTES.sellerDashboard} className="flex items-center gap-3">
      ...
      <nav aria-label="Seller" className="flex-1 space-y-5 overflow-y-auto px-3 py-4" onClick={onNavigate}>
          ...
          <div key={group.label} className="mb-1">
            ...
            <div className="space-y-0.5">
              {group.items.map((item) => {
                const active =
                  pathname === item.href ||
                  (item.href !== ROUTES.sellerDashboard && pathname.startsWith(`${item.href}/`));
      ...
      <div className="flex-shrink-0 border-t border-sidebar-border px-3 py-4">
        <Link
          href={ROUTES.profile}
          className="mb-2 flex items-center gap-3 rounded-lg px-3 py-2.5 transition-colors hover:bg-sidebar-border/50"
        >
