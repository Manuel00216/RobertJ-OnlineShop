...
| TD-11 | **Xendit-dashboard-initiated refunds are not reflected back automatically** — no `/refunds` webhook handler exists | A refund processed directly in the Xendit dashboard would not update `orders.payment_status`/`payments.status` here; the existing `decide_return` admin flow is the only way to record a refund, and does so independent of how the money was actually returned | Add a `process_xendit_refund_webhook` RPC (mirroring `process_xendit_webhook`) if/when this goes into production — deliberately out of scope for the current sandbox implementation (see [ADR-019](./DECISIONS.md#adr-019-xendit-online-payment-gcash-maya-card-superseding-adr-008s-no-gateway-stance)) |
